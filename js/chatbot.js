...
const FAQ_DATABASE = {
    ...
    'karriere': {
        ...
        answer: 'Mein Weg in Kurzform: 2004–2008 Ausbildung Mediamatiker. Dann MySign, wo ich vom Frontend-Entwickler zum Teamleiter Marketing aufgestiegen bin. 2016 zu siroop (Swisscom/Coop), dort kam der erste Kontakt mit generativer KI. Danach CEO von RecruitingHUB, KI im HR. 2019 hab ich BEYONDER gegründet, 2023 swissAI. 2025 wurde BEYONDER zur AG. Und 2026 die Digital Shaper Auszeichnung. Kein gerader Weg, aber jeder Schritt hat mich hierhin gebracht.'
    },
    'swissai': {
        keywords: ['swissai', 'kimpact', 'verein', 'verband', 'schweiz', 'ki verband'],
        answer: 'swissAI ist der Schweizer Verband für Künstliche Intelligenz, den ich 2023 gegründet habe (damals noch als KImpact). Rund 300 Mitglieder, gemeinnützig. Unsere Mission: Wissen teilen, Dialog fördern und verantwortungsvolle Innovation vorantreiben. Mehr auf swissai.ch'
    },
    'beyonder': {
    ...
    'digital_shaper': {
        ...
        answer: '2026 wurde ich als Digital Shaper ausgezeichnet, von Ringier, Bilanz und Handelszeitung in der Kategorie «AI Acrobats». Die Digital Shapers ehren Persönlichkeiten, die die Digitalisierung in der Schweiz vorantreiben. Freut mich natürlich, aber die eigentliche Arbeit zählt mehr als der Titel.'
    },
    'dozent': {
    ...
    'essay': {
        ...
        answer: 'Ich hab ein Essay geschrieben: «Gestalte deine Zukunft mit Künstlicher Intelligenz.» Nicht mit Angst und blindem Vertrauen, sondern mit kritischem Denken und aktivem Handeln. Das ist meine Haltung in Textform. Lies es auf chrisbeyeler.ch/essay/2025/'
    },
    'medien': {
        keywords: ['medien', 'presse', 'interview', 'srf', 'zeitung', 'pressekit', 'foto'],
        answer: 'Du findest mich in SRF, Ringier/Bilanz, persoenlich.com, Aargauer Zeitung, Handelszeitung, topsoft, Netzwoche und weiteren Medien. Das Pressekit mit Bio-Texten und Profi-Fotos gibts auf chrisbeyeler.ch. Für Medienanfragen: chris@beyonder.ch'
    },
    'core': {
        keywords: ['core', 'framework', 'prompt', 'prompting', 'methode', 'skill-stack'],
        answer: 'CORE+ ist unser eigenes Prompt-Framework bei BEYONDER. Stell es dir vor wie ein Navi für den Prompting-Prozess: strukturiert, klar, wirkungsvoll. Das Plus steht für «bleib im Gespräch», weil gute Ergebnisse durch Zusammenarbeit entstehen. Auch mit KI.'
    },
    'format': {
    ...
    'default': {
        answer: 'Ich bin die digitale Version von Chris Beyeler und kann dir Infos geben zu: meiner Person, Keynotes, BEYONDER AG, swissAI, dem AI Cast Podcast oder KI-Workshops. Was interessiert dich? Für alles Weitere: chris@beyonder.ch'
    }
};
...
function addTypingIndicator() {
    ...
    msg.className = 'chatbot__message chatbot__message--bot chatbot__message--typing';
    msg.innerHTML = '<div class="chatbot__typing-dots"><span></span><span></span><span></span></div>';
...
(function() {
    ...
    const chatFullQuickReplies = document.getElementById('chatFullQuickReplies');
    if (!chatFullMessages || !chatFullInput) return;

    let chatFullHistory = [];

    function addFullTypingIndicator() {
        const el = document.createElement('div');
        el.className = 'chat-full__typing';
        el.innerHTML = '<span></span><span></span><span></span>';
        chatFullMessages.appendChild(el);
        chatFullMessages.scrollTop = chatFullMessages.scrollHeight;
        return el;
    }

    function addFullMessage(text, sender) {
        const msg = document.createElement('div');
        msg.className = 'chat-full__message chat-full__message--' + (sender === 'user' ? 'user' : 'bot');
        const p = document.createElement('p');
        if (sender === 'bot') {
            p.innerHTML = renderMarkdown(text);
        } else {
        ...
        msg.appendChild(p);
        chatFullMessages.appendChild(msg);
        chatFullMessages.scrollTop = chatFullMessages.scrollHeight;
    }

    async function sendFullMessage() {
        const text = chatFullInput.value.trim();
        if (!text) return;

        addFullMessage(text, 'user');
        chatFullInput.value = '';
        chatFullHistory.push({ role: 'user', content: text });

        if (chatFullQuickReplies) chatFullQuickReplies.style.display = 'none';

        const typingEl = addFullTypingIndicator();
        ...
        if (apiAvailable !== false) {
            try {
                ...
                const response = await fetch(CHATBOT_API, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ messages: chatFullHistory.slice(-10) }),
                    signal: controller.signal
                });
                clearTimeout(timeoutId);
                if (!response.ok) throw new Error('API error');
                ...
                const reply = data.reply || findFAQAnswer(text);
                typingEl.remove();
                addFullMessage(reply, 'bot');
                chatFullHistory.push({ role: 'assistant', content: reply });
                apiAvailable = true;
                return;
            } catch (err) {
                apiAvailable = false;
    ...
    chatFullSend.addEventListener('click', sendFullMessage);
    chatFullInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') sendFullMessage();
    });

    if (chatFullQuickReplies) {
        chatFullQuickReplies.querySelectorAll('.chatbot__quickreply').forEach(btn => {
            btn.addEventListener('click', () => {
                chatFullInput.value = btn.getAttribute('data-question');
                chatFullQuickReplies.style.display = 'none';
                sendFullMessage();
            });
        });
    }
